    ...
    switch ($paso) {
        ...
        case 4:
            ...
            foreach ($_SESSION['form']['pasos'][2]['values'] as $musculo) {
                ...
                $peso = $_SESSION['form']['pasos'][3]['values']['peso_' . $musculo];
                $repeticiones = $_SESSION['form']['pasos'][3]['values']['repeticiones_' . $musculo];

                foreach ($_SESSION['form']['pasos'][4]['options'][$musculo] as $index => $value) {
                    $formBody .= "<li>
                    <label for='plan_." . $index . ">Plan para " . $musculo . " " . $index . "</label>
                    <input type='radio' name='plan_" . $musculo . " id='plan_" . $index . "' value='plan_" . $index . "'>";
...
function joinPesoYRepes($pesos, $repes)
{
    ...
    foreach ($pesos as $key => $value) {
        $tempArray = ["peso_" . $value, "repeticiones_" . $repes[$key]];
    }
}
